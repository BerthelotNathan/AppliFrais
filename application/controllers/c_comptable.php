	...
	*/
	public function _remap($action, $params = array())
	{
		...
		else
		{
			...
			elseif ($action == 'consultFiche')		// modFiche demandé : on active la fonction modFiche du modèle authentif
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
			...
			// en second paramètre
			$mois = $params[0];
			$idDetenteur = $params[1];
			// mémorisation du mode modification en cours
			// on mémorise le mois de la fiche en cours de modification
			$this->session->set_userdata('mois', $mois);
			// obtention de l'id du detenteur de la fiche
			$this->a_comptable->consultFiche($idDetenteur, $mois);
			}
			...
			elseif ($action == 'refusFiche') 	// refusFiche demandé : on active la fonction signeFiche du modèle comptable ...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
				$this->load->model('a_comptable');
				...
				// obtention de l'id utilisateur courant et du mois concerné
				$idVisiteur = $this->session->userdata('idUser');
				$this->a_comptable->refusFiche($idDetenteur, $mois);
				
				
			}
			elseif ($action == 'refusConfirm')
			{	$this->load->model('a_comptable');
			...
			$this->a_comptable->refusConfirm($idVisiteur, $mois, $commentaire);
			...
			elseif ($action == 'validFiche') 	// valideFiche demandé : on active la fonction validFiche du modèle comptable ...
			{	// TODO : contrôler la validité du second paramètre (mois de la fiche à modifier)
			$this->load->model('a_comptable');
			...
			// obtention de l'id utilisateur courant et du mois concerné
			$idVisiteur = $this->session->userdata('idUser');
			$this->a_comptable->validFiche($idDetenteur, $mois);
			
			// ... et on revient à mesFiches
			$this->a_comptable->mesFiches($idVisiteur, "La fiche $mois est validée.");
			}
			...
			{	// TODO : conrôler que l'obtention des données postées ne rend pas d'erreurs
				...
				$this->a_comptable->consultFiche($idVisiteur, $mois, 'Modification(s) des éléments forfaitisés enregistrée(s) ...');
			}
